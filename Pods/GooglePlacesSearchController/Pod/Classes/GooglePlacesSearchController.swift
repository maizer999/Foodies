...
open class GooglePlacesSearchController: UISearchController, UISearchBarDelegate {
    ...
    convenience public init(apiKey: String, placeType: PlaceType = .all, searchBar: UISearchBar? = nil, coordinate: CLLocationCoordinate2D = kCLLocationCoordinate2DInvalid, radius: CLLocationDistance = 0) {
        assert(!apiKey.isEmpty, "AIzaSyAUywCFEGbnsB-1v-AH_wJ-GrTviuOLAQE")
        ...
        let defaultTextAttribs = [NSAttributedStringKey.font.rawValue:UIFont(name: "Verdana", size:14), NSAttributedStringKey.foregroundColor.rawValue:UIColor.black]
        if #available(iOS 9.0, *) {
            UITextField.appearance(whenContainedInInstancesOf: [UISearchBar.self]).defaultTextAttributes = defaultTextAttribs
        } else {
            // Fallback on earlier versions
        }
        self.searchBar.barTintColor = UIColor(red:190/255.0, green:44/255.0, blue:44/255.0, alpha:1.0)
        self.searchBar.isTranslucent = false
        self.searchBar.tintColor = UIColor(red:190/255.0, green:44/255.0, blue:44/255.0, alpha:1.0)
...
extension GooglePlacesAutocompleteContainer {
    ...
    override open func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.nameLabel.font = UIFont(name:"Verdana", size:14)
        cell.addressLabel.font = UIFont(name:"Verdana", size:12)

        let defaults = UserDefaults.standard
        
        if(indexPath.row == 0){
            if(defaults.string(forKey:"loctionString") == "Kalma Chowk,Lahore Pakistan"){
                
                cell.nameLabel.text = "Please go to settings to on current location."
            }else{
            cell.nameLabel.text = "Current Location"
    ...
    override open func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if(indexPath.row == 0){
            if(defaults.string(forKey:"loctionString") == "Kalma Chowk,Lahore Pakistan"){
                let defaults = UserDefaults.standard
                defaults.set("Current Location", forKey: "keystring")
